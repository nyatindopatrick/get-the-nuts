...
class playGame extends Phaser.Scene {
  ...
  create() {
    ...
    this.coinGroup = this.add.group({
      // once a nut is removed, it's added to the pool
      removeCallback: function (nut) {
        nut.scene.coinPool.add(nut);
      },
    });

    // nut pool
    this.coinPool = this.add.group({
      // once a nut is removed from the pool, it's added to the active coins group
      removeCallback: function (nut) {
        nut.scene.coinGroup.add(nut);
      },
    });
    ...
    // setting collisions between the player and the nut group
    this.physics.add.overlap(
      ...
      function (player, nut) {
        this.tweens.add({
          ...
          callbackScope: this,
          onComplete: function () {
            this.coinGroup.killAndHide(nut);
            this.coinGroup.remove(nut);
          },
        });
      },
      null,
  ...
  addPlatform(platformWidth, posX, posY) {
    ...
    if (this.addedPlatforms > 1) {
      // is there a nut over the platform?
      if (Phaser.Math.Between(1, 100) <= gameOptions.coinPercent) {
        if (this.coinPool.getLength()) {
          let nut = this.coinPool.getFirst();
          nut.x = posX;
          nut.y = posY - 96;
          nut.alpha = 1;
          nut.active = true;
          nut.visible = true;
          this.coinPool.remove(nut);
        } else {
          let nut = this.physics.add.sprite(posX, posY - 96, "nut");
          nut.setImmovable(true);
          nut.setVelocityX(platform.body.velocity.x);
          nut.anims.play("rotate");
          nut.setDepth(2);
          this.coinGroup.add(nut);
        }
      }
  ...
  update() {
    ...
    // recycling coins
    this.coinGroup.getChildren().forEach(function (nut) {
      if (nut.x < -nut.displayWidth / 2) {
        this.coinGroup.killAndHide(nut);
        this.coinGroup.remove(nut);
      }
    }, this);
